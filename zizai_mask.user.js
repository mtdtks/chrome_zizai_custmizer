...
//先頭<script>>機能がなぜかオフになってしまうので改めてこちらに追記する。
//恐らく他の機能もオフになっているかもなので必要であればこちらに持ってくれば動くはず。

//　<!-- デザイン確認のため、強制的にcssを読み込ませる -->

window.onload = function() {
    var css=document.createElement("link");
    css.setAttribute("rel","stylesheet");
    css.setAttribute("type","text/css");
    css.setAttribute("href","/user_data/packages/admin/css/admin_contents_product.css");
...
$(function () {
    ...
    // 本体価格から税込価格を計算する
    $("input[name='price01']").blur(function(){
        if($(this).val() !== "" &&                   // 自身が入力されている
          $(this).val().match(/^[0-9]+$/) &&        // 数値のみが入力されている
          $("input[name='price02']").val() === ""){  // 税込価格が入力されていない
            $("input[name='price02']").val(
                    Math.floor($(this).val() * (1 + 0.08)));
        }
    });
    /*
    // ローディング
...
// 表示非表示切り替え
function lfDispSwitch(id, title) {
    var obj = document.getElementById(id);
    if (obj.style.display == 'none') {
        obj.style.display = '';
        $('#' + id + '_btn').text(title + '非表示');
    } else {
        obj.style.display = 'none';
        $('#' + id + '_btn').text(title + '表示');
    }
}
...
function fnInitSelect(select) {
    ...
    for(var i=0; i<selectedOptions.length; i++){
        $('#' + select + ' option[value="' + selectedOptions[i] + '"]').attr('selected', 'selected');
    }
}
...
function getMasterUrl() {
    return '/koshoadmin/products/master.php?search_isbn=' + $("#master_search_isbn").val().replace(/[\n\r]/g, "") + '&is_popup=true&is_regist=true&mode=search';
}
function getMasterNameUrl() {
    return '/koshoadmin/products/master.php?search_isbn=' + $("#master_search_isbn").val().replace(/[\n\r]/g, "") + '&search_name=' + encodeURI($("#master_search_name").val()) + '&search_author=' + encodeURI($("#master_search_author").val()) + '&search_publisher=' + encodeURI($("#master_search_publisher").val()) + '&is_popup=true&is_regist=true&mode=search';
}

function getEcProductListUrl() {
    return 'https://www.kosho.or.jp/products/list.php?\mode=search_header&search_only_has_stock=1&search_name=' + encodeURI(document.form1.name.value) + '&admin=on';
}

function getAmazonProductListUrl() {
    return 'http://www.amazon.co.jp/gp/search/?field-keywords=' + encodeURI(document.form1.name.value);
}

function previewProductName() {
    var name = $('textarea[name=name]').val().replace(/\r?\n/g, '');
    var vol = $('input[name=volume]').val();
    vol = (vol == "null" || vol.length === 0) ? "" : " "+vol;
    var stitle = $('input[name=series_titles]').val();
    stitle = (stitle == "null" || stitle.length === 0) ? "" : " "+stitle;
    var svol = $('input[name=series_titles_volume]').val();
    svol = (svol == "null" || svol.length === 0) ? "" : " "+svol;
    var label = $('input[name=label]').val();
    label = (label == "null" || label.length === 0) ? "" : " "+label;
    var edition = $('input[name=edition]').val();
    edition = (edition == "null" || edition.length === 0) ? "" : " "+edition;
    var series = stitle + svol + label;
    if (series.length !== 0) {
        series = " <" + series + " >";
    }
    alert(name + vol + series + edition);
}
